...
export default function CheckoutModal({
    ...
    useEffect(() => {
        ...
        if (typeof authService.getUser === 'function') {
            authService.getUser()
                .then(u => setCashierName(u?.user?.name ?? u?.user?.email ?? fromJwt()))
    ...
    const handleProcessSale = async () => {
        ...
        if (paymentMethod === 'cash' && (!amountPaid || isNaN(paid) || paid < total)) {
                ...
                <div className="p-6 overflow-y-auto flex-1 min-h-0">
                    ...
                    {saleCompleted ? (
                        <div className="flex flex-col items-center text-center gap-4">
                            ...
                            <button onClick={handleFinishAndClear}
                        ...
                        <div className="grid gap-6">
                            ...
                            <div className="space-y-3">
                                <label className="text-sm font-bold text-slate-600 block">Método de pago</label>
                                <div className="grid grid-cols-2 gap-3">
                                    {[
                                        ...
                                        <button key={m.value}
                                            ...
                                            className={`py-4 rounded-2xl border-2 font-bold flex items-center justify-center gap-2 transition-all ${paymentMethod === m.value
                            ...
                            {paymentMethod === 'cash' && (
                                <div className="space-y-2">
                                    <label className="text-sm font-bold text-slate-600 block">Efectivo recibido</label>
                                    <div className="relative">
                                        ...
                                        <input
                                            ...
                                            className="w-full pl-9 pr-4 py-4 rounded-2xl border-2 border-slate-200 bg-white text-2xl font-black focus:outline-none focus:border-slate-900 focus:ring-4 focus:ring-slate-900/10 transition-all"
                                            placeholder="0.00"
                                        />
                                    </div>
                                    ...
                                    <div className="grid grid-cols-4 gap-2">
                                        {[...new Set([
                                            ...
                                            <button key={v}
                                                ...
                                                className="py-2 rounded-xl border border-slate-200 text-sm font-bold text-slate-700 hover:bg-slate-50 transition-colors">
                                                ${v % 1 === 0 ? v : v.toFixed(2)}
                                            </button>
                                        ))}
                                    </div>
                                    ...
                                    {Number(amountPaid) > 0 && (
                                        <div className={`rounded-xl px-4 py-3 flex justify-between items-baseline ${Number(amountPaid) >= total
                                            ? 'bg-emerald-50 border border-emerald-200'
                                            : 'bg-red-50 border border-red-200'}`}>
                                            <span className="text-sm font-bold text-slate-600">Cambio</span>
                                            <span className={`text-2xl font-black ${Number(amountPaid) >= total ? 'text-emerald-600' : 'text-red-600'}`}>
                            ...
                            <button
                                ...
                                className="w-full py-4 rounded-2xl bg-emerald-600 text-white text-xl font-black hover:bg-emerald-700 shadow-lg flex items-center justify-center gap-2 transition-all disabled:opacity-50 disabled:cursor-not-allowed">
                                {loading ? (
                                    <>
